...
save(predictions_list, file="models/Predictions_sub.Rdata")
load("models/Predictions_sub.Rdata")
...
text_x<-80
text_y<-seq(1.5, 1.35, length=4)
text_data<-data.frame(x=text_x, y=text_y, label=text_add)
y_upper<-c(1.5, 1.2, 1.2, 1.2)
...
for(m in 1:dim(manipulations)[1]){
	...
	if(m == 1){
		ratio<-ratio + 
				theme(legend.position=c(0.14, 0.87), legend.text=element_text(size=12), legend.background=element_blank()) + 
...
pdf("figures/Figure_7.pdf", height=15, width=15)
