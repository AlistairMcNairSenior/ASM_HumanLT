...
# Murray et al 2003 (eq 4.) say that:

# logit(lx_ij) = a_ij _+ b_ij * logit(lx_s) + gamma_x * (1 - logit(l5_ij) / logit(l5_s)) + theta_x * (1 - logit(l60_ij) / logit(l60_s))

# I believe this can be estimated as a random LMM:

# lmer(lx ~  lsx + (1 + lsx|table) + offset(v2) + offset(v3))
# where v2 and v3 have been calculated before hand for each observation
# This can also be done sex-specifically by adding Sex with interaciton with lsx and calcualting v2 and v3 sex-specifically
...
LT_data<-read.csv("cleandata/Clean_LT.csv")
...
FB_data<-read.csv("cleandata/Clean_FB.csv")
...
SES_data<-read.csv("cleandata/Clean_GDP_Mad.csv")
head(SES_data)
...
SES_data2<-read.csv("cleandata/Clean_GDP_WB.csv")
head(SES_data2)
...
# Add in the FB data
all_males<-cbind(all_males, FB_data[match(all_males$ID, FB_data$Food_ID), c("Protein.kcal", "Carbo.kcal", "Fat.kcal")])
...
write.table(complete_cases, file="Brass_complete_cases.csv", sep=",", row.names=F, col.names=names(complete_cases))
...
write.table(sub_data, file="Brass_subset.csv", sep=",", row.names=F, col.names=names(sub_data))
...
write.table(full_imp2, file="Brass_imputed.csv", sep=",", row.names=F, col.names=names(full_imp2))

# # pdf("Imputations.pdf", height=10, width=20)
# par(mfrow=c(2,4))

# countries<-unique(full_imp$Country)
# cols1<-c("Protein.kcal", "Carbo.kcal", "Fat.kcal", "GDP_perCapita", "GDP_perCapita_WB", "alpha", "beta")
# cols2<-paste0(cols1, "_imp")

# for(i in 1:length(countries)){
	
	# country_i<-full_imp2[which(full_imp$Country == countries[i]),]
	# for(j in 1:length(cols1)){
		# plot(country_i$Year, country_i[,cols1[j]], col=as.numeric(country_i[, cols2[j]])+1, main=countries[i], ylab=cols1[j], xlab="Year")
	# }
	# plot(-10, -10)
# }
