...
write.GAM(GAM_e0, csv.file="Table_S1_(e0_time).csv")
...
write.GAM(GAM_l5, csv.file="Table_S2_(l5_time).csv")

GAM_l60<-gam(l60 ~ s(Year, by=Sex, k=50) + Sex + s(Country, bs="re"), data=full_data, gamma=gamma, family="betar")
summary(GAM_l60)
gam.check(GAM_l60)
# Some heteorgeneity in residuals (likely unaccounted for covariates) - k significant, but EDF know where near k' and index pretty close to 1
write.GAM(GAM_l60, csv.file="Table_S3_(l60_time).csv")
...
long<-data.frame(Supply=c(males$Protein.kcal, males$Carbo.kcal, males$Fat.kcal), Year=rep(males$Year, 3), Country=rep(males$Country, 3), Nutrient=c(rep("Protein", dim(males)[1]), rep("Carbohydrate", dim(males)[1]), rep("Fat", dim(males)[1])))
...
write.GAM(GAM_supply, csv.file="Table_S4_(supply_time).csv")
...
gam.check(GAM_GDP)
# Some heteorgeneity in residuals (likely unaccounted for covariates) - k fine
write.GAM(GAM_GDP, csv.file="Table_S5_(GDP_time).csv")
...
cor_var<-c("Total", "Protein.kcal", "Carbo.kcal", "Fat.kcal", "GDP_perCapita", "e0")
cor_1970<-cor(full_data[which(full_data$Year == 1970), cor_var])
rownames(cor_1970)<-c("Total", "Protein", "Carb.", "Fat", "GDP", "Life Exp.")
colnames(cor_1970)<-c("Total", "Protein", "Carb.", "Fat", "GDP", "Life Exp.")
cor_2010<-cor(full_data[which(full_data$Year == 2010), cor_var])
rownames(cor_2010)<-c("Total", "Protein", "Carb.", "Fat", "GDP", "Life Exp.")
colnames(cor_2010)<-c("Total", "Protein", "Carb.", "Fat", "GDP", "Life Exp.")
...
pdf("Figure_1.pdf", height=15, width=12)
